...
exports.wikidataDishesService = void 0;
...
class WikidataDishesService {
    ...
    async getDishSuggestions(query) {
    ...
    async executeSimpleQuery(query, type) {
        ...
        let sparqlQuery;
        if (type === 'exact') {
            const exactFilters = accentVariations.map(variant => `LCASE(?dishLabel) = "${variant}"`).join(' || ');
            sparqlQuery = `
      ...
      `;
            ...
            const partialFilters = accentVariations.map(variant => `CONTAINS(LCASE(?dishLabel), "${variant}")`).join(' || ');
            sparqlQuery = `
        SELECT DISTINCT ?dish ?dishLabel WHERE {
          {
            # Direct instances of dish
    ...
    async executeSparqlQuery(sparqlQuery) {
        ...
        try {
            ...
            const dishes = data.results.bindings.map(binding => capitalizeWords(binding.dishLabel.value));
...
exports.wikidataDishesService = new WikidataDishesService();
//# sourceMappingURL=wikidataDishesService.js.map